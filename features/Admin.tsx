        ...
        <div className="hidden md:flex gap-2 mb-8 overflow-x-auto pb-2 scrollbar-hide">
          {tabs.map(tab => (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 rounded-full transition-all whitespace-nowrap text-sm font-medium border ${activeTab === tab.id
                ? 'bg-accent text-black font-bold border-accent shadow-[0_0_15px_rgba(244,201,93,0.3)]'
            ...
            >
              ...
              {tab.count !== undefined && tab.count > 0 && (
                <span className={`px-2 py-0.5 text-xs rounded-full font-bold ${activeTab === tab.id ? 'bg-black/20 text-black' : 'bg-accent/20 text-accent'
                  }`}>
                  {tab.count}
                </span>
              )}
