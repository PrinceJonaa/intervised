...
import type { Testimonial, Project } from '../lib/supabase/referenceService';
...
export const HomeView = ({ setPage }: { setPage: (p: Page) => void }) => {
  ...
  const dataLoadTriggerRef = useRef<HTMLDivElement | null>(null);
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        const { getProjects, getTestimonials } = await import('../lib/supabase/referenceService');
    ...
    const target = dataLoadTriggerRef.current;
    ...
    <div className="flex flex-col pb-32">
      ...
      <section className="min-h-[100dvh] flex flex-col justify-center items-center relative px-4 sm:px-6 py-12 sm:py-20">
        <div className="max-w-5xl w-full z-10 text-center mt-auto mb-auto">
          <div>
            <p className="text-accent font-mono text-[10px] sm:text-xs md:text-sm tracking-[0.3em] mb-4 sm:mb-6 uppercase">
              Mutually Envisioned
            </p>
            <h1 className="font-display text-5xl sm:text-7xl md:text-9xl font-bold leading-none tracking-tighter mb-6 break-words text-white">
          ...
          <p className="text-gray-200 max-w-xl mx-auto text-sm sm:text-lg leading-relaxed mb-8 sm:mb-10 px-4">
        ...
        {/* Scroll Indicator */}
        <div
          className="mt-8 sm:mt-16 text-gray-300 flex flex-col items-center gap-2 cursor-pointer pb-4 sm:pb-8 animate-bounce"
      ...
      <section className="py-16 sm:py-24 px-4 sm:px-6 relative z-10 bg-gradient-to-b from-transparent to-void/80">
        <div className="max-w-4xl mx-auto text-center mb-16 sm:mb-24">
          <h2 className="text-2xl sm:text-3xl md:text-5xl font-display font-bold mb-6 sm:mb-8 leading-tight text-white">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
            <div className="p-6 border-l border-white/20 bg-white/5 md:bg-transparent rounded-r-xl md:rounded-none">
              <h3 className="font-bold text-xl mb-2 text-accent">Creative</h3>
              <p className="text-sm text-gray-500">Videography, Photography, Music Production</p>
            </div>
            <div className="p-6 border-l border-white/20 bg-white/5 md:bg-transparent rounded-r-xl md:rounded-none">
              <h3 className="font-bold text-xl mb-2 text-white">Tech</h3>
              <p className="text-sm text-gray-500">Automation, AI Bots, OBS Setup</p>
            </div>
            <div className="p-6 border-l border-white/20 bg-white/5 md:bg-transparent rounded-r-xl md:rounded-none">
              <h3 className="font-bold text-xl mb-2 text-muted">Ministry</h3>
        ...
        {/* Recent Work Grid */}
        <div ref={dataLoadTriggerRef} className="max-w-6xl mx-auto mb-16 sm:mb-24">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 px-0 sm:px-4">
            {projects.length > 0 ? (
              ...
              Array.from({ length: 2 }).map((_, idx) => (
                <div key={`project-skeleton-${idx}`} className="bg-white/5 border border-white/10 rounded-2xl p-6 animate-pulse">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {testimonials.length > 0 ? (
              ...
              Array.from({ length: 3 }).map((_, idx) => (
                <div key={`testimonial-skeleton-${idx}`} className="bg-gradient-to-br from-white/5 to-transparent p-6 rounded-2xl border border-white/5 animate-pulse">
                  <div className="h-5 w-5 rounded bg-white/20 mb-4" />
                  <div className="h-4 w-full bg-white/10 rounded mb-2" />
